...
const Adds = ({ event, navigation }) => {

  if (!event || !event.img) {
    return null; // o muestra un fallback si quieres
  }

  let banner = '';
  try {
    const parsed = JSON.parse(event.img);
    banner = Array.isArray(parsed) ? parsed[0] : '';
  } catch (e) {
    console.error('Error al parsear event.img:', e);
  }
